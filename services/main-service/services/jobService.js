...
const insertJobWithSkills = async (jobData) => {
    ...
    try {
      ...
      const insertJobQuery = `
        INSERT INTO job (job_id, profile_id, job_name, experience, education_required)
        VALUES (?, ?, ?, ?, ?)
      `;
      await conn.execute(insertJobQuery, [
        job_id,
        profile_id,
        job_name,
        experience,
      ...
      for (const skill of skills_required) {
        await conn.execute(insertSkillQuery, [job_id, skill]);
      }
  ...
  module.exports = { insertJobWithSkills , getAllJobs , updateJob , deleteJob, getJob };